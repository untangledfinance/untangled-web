...
async function main() {
  try {
    ...
    const modelVersion =
      data.model?.version && data.model.version !== 'null'
        ? data.model.version
        : '';
    ...
    try {
      ...
      if (blocksOutput) {
        ...
        if (activeBlock) {
          ...
          const resetTimeStr =
            activeBlock.usageLimitResetTime || activeBlock.endTime;
    ...
    // Directory
    output += `ð ${DirColor}${currentDir}${Reset}`;

    // Git branch
    if (gitBranch) {
      output += `  ð¿ ${GitColor}${gitBranch}${Reset}`;
    }

    // Model
    output += `  ð¤ ${ModelColor}${modelName}${Reset}`;

    // Model version
    if (modelVersion) {
      output += `  ð·ï¸ ${VersionColor}${modelVersion}${Reset}`;
    }

    // Session time
    if (sessionText) {
      const sessionColorCode = getSessionColor(sessionPercent);
      output += `  â ${sessionColorCode}${sessionText}${Reset}`;
    }
    ...
    if (costUSD && /^\d+(\.\d+)?$/.test(costUSD)) {
      ...
      if (costPerHour && /^\d+(\.\d+)?$/.test(costPerHour)) {
        ...
        output += `  ðµ ${CostColor}$${costUSDNum.toFixed(2)} ($${costPerHourNum.toFixed(2)}/h)${Reset}`;
      } else {
        output += `  ðµ ${CostColor}$${costUSDNum.toFixed(2)}${Reset}`;
      }
    }

    // Tokens
    if (totalTokens && /^\d+$/.test(totalTokens.toString())) {
      output += `  ð ${UsageColor}${totalTokens} tok${Reset}`;
    }
