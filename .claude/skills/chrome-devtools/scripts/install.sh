...
echo "ð Installing Chrome DevTools Agent Skill..."
echo ""
...
if [ "$NODE_VERSION" -lt 18 ]; then
  echo "â Error: Node.js 18+ is required. Current version: $(node --version)"
...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
  ...
  if [ ${#MISSING_DEPS[@]} -gt 0 ]; then
    echo "â ï¸  Missing system dependencies: ${MISSING_DEPS[*]}"
    echo ""
    ...
    echo "â System dependencies found"
  fi
  echo ""
