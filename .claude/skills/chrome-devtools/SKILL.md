...
â Wrong working directory â output files go to wrong location
â Skipping output validation â silent failures
â Using complex CSS selectors without testing â selector errors
â Not checking element visibility â timeout errors

â Always verify `pwd` before running scripts
â Always validate output after screenshots
â Use snapshot.js to discover selectors
â Test selectors with simple commands first
...
if (( $(echo "$LCP < 2500" | bc -l) )); then
  echo "â LCP passed: ${LCP}ms"
else
  echo "â LCP failed: ${LCP}ms"
fi
```
