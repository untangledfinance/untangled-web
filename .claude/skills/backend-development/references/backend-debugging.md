...
```typescript
// â Bad: Blocking event loop
function fibonacci(n) {
  if (n <= 1) return n;
  return fibonacci(n - 1) + fibonacci(n - 2); // Exponential time
}

// â Good: Memoized or iterative
const memo = new Map();
...
```typescript
// â Memory leak: Event listeners not removed
class DataService {
...
// â Memory leak: Global cache without limits
const cache = new Map();
...
// â Fix: LRU cache with size limit
...
// â Fix: Atomic operations (Redis)
async function incrementCounter() {
  return await redis.incr('counter');
  // Atomic, thread-safe
}

// â Fix: Database transactions
