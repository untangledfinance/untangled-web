...
- Handles fallback: topic â general â error
...
3. **Automatic fallback** - Scripts handle topic â general â error chains
...
```bash
node scripts/detect-topic.js "<query>"  # â {topic, library, isTopicSpecific}
node scripts/fetch-docs.js "<query>"    # â 2-3 URLs
...
```bash
node scripts/detect-topic.js "<query>"         # â {isTopicSpecific: false}
node scripts/fetch-docs.js "<query>"           # â 8+ URLs
cat llms.txt | node scripts/analyze-llms-txt.js -  # â {totalUrls, distribution}
