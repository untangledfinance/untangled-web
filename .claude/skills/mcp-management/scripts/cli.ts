...
async function main() {
  ...
  try {
    ...
    console.log('â Connected to all MCP servers\n');
...
async function listTools(manager: MCPClientManager) {
  ...
  for (const tool of tools) {
    console.log(`ð¦ ${tool.serverName} / ${tool.name}`);
  ...
  try {
    ...
    console.log(`\nâ Tools saved to ${toolsPath}`);
  } catch (error) {
    console.error(`\nâ Failed to save tools: ${error}`);
  }
}
...
async function listPrompts(manager: MCPClientManager) {
  ...
  for (const prompt of prompts) {
    console.log(`ð¬ ${prompt.serverName} / ${prompt.name}`);
...
async function listResources(manager: MCPClientManager) {
  ...
  for (const resource of resources) {
    console.log(`ð ${resource.serverName} / ${resource.name}`);
