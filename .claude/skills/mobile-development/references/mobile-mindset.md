...
- **Action**: MVP â User feedback â Iterate
...
### Small Screens (Constraint â Design Parameter)
...
```
â Bad: 8-column data table on mobile
â Good: Card view with 3 key metrics, "View more" for details
```
...
```
â Bad: Load all 1000 items in list
â Good: Virtualized list (10 items visible + buffer)
```
...
```
â Bad: Show spinner while posting comment
â Good: Show comment immediately (optimistic), sync background, handle conflicts
```
...
```
Launch Time
ââ Cold start: <2s (target 1.5s)
ââ Warm start: <1s
ââ Hot start: <0.5s

Screen Load
ââ Cached data: <500ms
ââ Network data: <2s
ââ Heavy computation: <3s

Memory
ââ Typical screen: <100MB
ââ Heavy screen (images): <150MB
ââ Peak usage: <200MB

Network
ââ Initial bundle: <2MB
ââ Per screen: <500KB
ââ Images: <200KB each

Battery
ââ Active use: <5% per hour
ââ Background: <1% per hour
ââ Idle: <0.1% per hour
```
...
```
Week 1: Build + Test
ââ Day 1-2: Design + plan
ââ Day 3-4: Implement core
ââ Day 5: Code review + tests

Week 2: Polish + Ship
ââ Day 6-7: Bug fixes + polish
ââ Day 8: QA testing
ââ Day 9: Staging deployment
ââ Day 10: Production release (staged)
```
...
**Problem**: Memory leaks â crashes, poor performance
...
ââ Show placeholder immediately
ââ Load low-res image (10KB)
ââ Display with smooth fade-in

Enhancement (modern devices):
ââ Check network (fast = high-res)
ââ Check memory (ample = cache)
ââ Progressive JPEG rendering
```
...
ââ Complex particle effects
ââ 120 FPS animations
ââ Parallax scrolling

Degraded (budget devices):
ââ Simple fade transitions
ââ 60 FPS target
ââ Disable parallax (GPU load)
```
...
```javascript
// React Native
const isLowEndDevice = DeviceInfo.getTotalMemory() < 2000000000; // <2GB
...
- **Yes** â Flutter
- **No** â React Native (easier for standard UIs)

**Q5: Existing codebase to share?**

- **React web app** â React Native
- **No existing code** â Flutter (cleaner slate)
...
ââ UI and business logic
ââ Standard features
ââ API integration

Native Modules (10%)
ââ Performance-critical (video processing)
ââ Platform-specific (HealthKit, Android Auto)
ââ Third-party SDKs (payment, analytics)
```
...
ââ Focus: Ship fast, validate idea

v2.0: Add global state when needed
ââ Trigger: Props drilling becomes painful

v3.0: Add Clean Architecture when scaling
ââ Trigger: Team grows, features multiply

v4.0: Extract microservices if justified
ââ Trigger: Independent deployment needs
```
