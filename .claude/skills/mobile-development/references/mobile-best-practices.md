...
```
User opens screen
ââ Show cached data immediately (stale-while-revalidate)
ââ Fetch fresh data in background
ââ Update UI when fresh data arrives
```
...
```
User makes change
ââ Update local database immediately
ââ Update UI optimistically
ââ Queue sync operation
ââ Sync to server in background
```
...
```
Push Sync (Real-time)
ââ WebSocket connection for critical updates
ââ Immediate notification of changes

Pull Sync (Periodic)
ââ Periodic polling for non-critical data
ââ Pull on app foreground
ââ Incremental sync (only changes since last sync)
```
...
```
â Bad: Request permission on app launch
â Good: Request after user sees value
...
ââ JWT (Stateless auth tokens)
ââ Refresh tokens (Long-term access)
ââ Biometric (Convenient re-auth)
```
