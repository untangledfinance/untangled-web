...
```kotlin
// Conditional breakpoint
// Right-click breakpoint â Condition: userId == "123"
...
```
View â Tool Windows â Profiler â CPU
- Record trace
...
```
View â Tool Windows â Profiler â Memory
- Track allocations
- Heap dump analysis
- Find memory leaks
```
...
```
View â Tool Windows â Profiler â Network
- All HTTP requests
...
# Settings â Network â Network Profile
...
# In app: Shake device â "Debug with React DevTools"
```
...
// Ctrl+Shift+P â "Toggle Debug Painting"
...
// Settings â Developer Options â Show layout bounds
```
...
```javascript
// â Bad: Heavy computation in render
function UserList({ users }) {
  const sortedUsers = users.sort((a, b) => a.name.localeCompare(b.name));
  return <FlatList data={sortedUsers} />;
}

// â Good: Memoize expensive operations
function UserList({ users }) {
...
// â Bad: ScrollView with large data
<ScrollView>
  {users.map(user => <UserCard key={user.id} user={user} />)}
</ScrollView>

// â Good: FlatList with virtualization
<FlatList
  data={users}
...
// â Bad: Event listener not removed
useEffect(() => {
...
2. Configure device proxy: Settings â WiFi â Modify â Proxy
...
```
Emulator: Settings â Network â Network Profile
```
...
```
Play Console â Quality â Crashes & ANRs
- Crash stack traces
- Affected devices
- OS versions
```
...
```dart
// â Bad: Not calling setState
void addUser(User user) {
  users.add(user); // Won't rebuild
}

// â Good: Call setState
void addUser(User user) {
  setState(() {
    users.add(user);
  });
...
```javascript
// â Bad: Navigation prop not available
function MyComponent() {
  navigation.navigate('Home'); // Error
}

// â Good: Use hook or prop
