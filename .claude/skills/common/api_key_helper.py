...
def find_api_key(skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    if api_key:
        print("â Using API key from environment variable", file=sys.stderr)
        return api_key
    ...
    if project_env.exists():
        ...
        if api_key:
            print(f"â Using API key from {project_env}", file=sys.stderr)
            return api_key
    ...
    if claude_env.exists():
        ...
        if api_key:
            print(f"â Using API key from {claude_env}", file=sys.stderr)
            return api_key
    ...
    if claude_skills_env.exists():
        ...
        if api_key:
            print(f"â Using API key from {claude_skills_env}", file=sys.stderr)
            return api_key
    ...
    if skill_env.exists():
        ...
        if api_key:
            print(f"â Using API key from {skill_env}", file=sys.stderr)
            return api_key
...
def get_api_key_or_exit(skill_dir: Optional[Path] = None) -> str:
    ...
    if not api_key:
        print("\nâ Error: GEMINI_API_KEY not found!", file=sys.stderr)
        print("\nð Please set your API key using one of these methods (in priority order):", file=sys.stderr)

        if skill_dir is None:
            skill_dir = Path(__file__).parent.parent
        project_dir = skill_dir.parent.parent.parent

        print("\n1ï¸â£  Environment variable (recommended for development):", file=sys.stderr)
        print("   export GEMINI_API_KEY='your-api-key'", file=sys.stderr)

        print("\n2ï¸â£  Project root .env file:", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.env", file=sys.stderr)

        print("\n3ï¸â£  .claude/.env file:", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.claude/.env", file=sys.stderr)

        print("\n4ï¸â£  .claude/skills/.env file (shared across all Gemini skills):", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.claude/skills/.env", file=sys.stderr)

        print("\n5ï¸â£  Skill directory .env file:", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {skill_dir}/.env", file=sys.stderr)

        print("\nð Get your API key at: https://aistudio.google.com/apikey", file=sys.stderr)
        print("\nð¡ Tip: Add .env files to .gitignore to avoid committing API keys", file=sys.stderr)
        sys.exit(1)
...
def get_client(skill_dir: Optional[Path] = None):
    ...
    if vertex_config['use_vertex']:
        ...
        if not vertex_config['project_id']:
            print("\nâ Error: VERTEX_PROJECT_ID required when GEMINI_USE_VERTEX=true!", file=sys.stderr)
            print("\nð Set your GCP project ID:", file=sys.stderr)
        ...
        print(f"â Using Vertex AI endpoint", file=sys.stderr)
...
if __name__ == '__main__':
    ...
    print(f"â Found API key: {api_key[:8]}..." + "*" * (len(api_key) - 8))
