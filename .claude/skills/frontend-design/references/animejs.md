...
## ð¨ CRITICAL: ALWAYS USE ANIME.JS V4 SYNTAX ð¨
...
## ð Quick Start - Essential Setup

### 1. Correct v4 Import (REQUIRED)

```javascript
// â CORRECT v4 imports
import {
  animate,
  createTimeline,
  eases,
...
// â WRONG v3 import - NEVER USE THIS
...
```javascript
// â ï¸ IMPORTANT: Set this ONLY ONCE in your app's main entry point
...
```javascript
// â GOOD - Clean, readable, one line for simple tweens
animate('.element', { x: 250, duration: 1, ease: 'outQuad' });

// â BAD - Unnecessary multi-line for simple tweens
animate('.element', {
  x: 250,
  duration: 1,
  ease: 'outQuad',
...
## ð¯ Core API - Most Common Patterns
...
```javascript
// â WRONG - 'targets' is v3
animate({ targets: '.element', translateX: 250 });

// â CORRECT - First parameter is the target
animate('.element', { x: 250 });
```
...
```javascript
// â WRONG - anime.timeline() is v3
const tl = anime.timeline();

// â CORRECT - Use createTimeline
import { createTimeline } from 'animejs';
const tl = createTimeline();
```

## ð Property Syntax Reference (v3 â v4)

### Animation Values

```javascript
// â v4: Use 'to' for target values
{
  opacity: {
    to: 0.5;
  }
...
// â v3: DON'T use 'easing' or 'ease' prefix
...
```javascript
// â v4: Simple callback - keep on one line
animate('.element', {
  x: 250,
  duration: 1,
...
// â v4: Multiple callbacks - use multi-line
animate('.element', {
  x: 250,
  duration: 1,
...
// â v3: DON'T use unprefixed callbacks
// { update: () => {}, complete: () => {} }
```

## ð Code Formatting Guidelines
...
```javascript
// â GOOD - Clean, readable, one line
animate('.element', { x: 250, duration: 1, ease: 'outQuad' });
...
1. **Use transforms over position properties**
   ...
   ```javascript
   // â Good - uses transform
   animate('.element', { x: 100 });

   // â Avoid - triggers layout
...
2. **Batch animations in timelines**
   ...
   ```javascript
   // â Good - single timeline
   const tl = createTimeline();
   elements.forEach((el) => tl.add(el, { x: 100 }));

   // â Avoid - multiple animations
...
## ð« How to Identify V3 Code (DON'T USE)
