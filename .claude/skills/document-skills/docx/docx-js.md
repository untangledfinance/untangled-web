...
// â WRONG: new TextRun("Line 1\nLine 2")
// â CORRECT: new Paragraph({ children: [new TextRun("Line 1")] }), new Paragraph({ children: [new TextRun("Line 2")] })
  ...
  sections: [
    {
      children: [
        ...
        // â ï¸ CRITICAL: Different reference = INDEPENDENT list that restarts at 1
...
// â ï¸ CRITICAL NUMBERING RULE: Each reference creates an INDEPENDENT numbered list
...
// â ï¸ CRITICAL: NEVER use unicode bullets - they create fake lists that don't work properly
// new TextRun("â¢ Item")           // WRONG
// new SymbolRun({ char: "2022" }) // WRONG
// â ALWAYS use numbering config with LevelFormat.BULLET for real Word lists
```
...
new Table({
  columnWidths: [4680, 4680], // â ï¸ CRITICAL: Set column widths at table level - values in DXA (twentieths of a point)
    ...
    new TableRow({
      ...
      children: [
        new TableCell({
          ...
          // â ï¸ CRITICAL: Always use ShadingType.CLEAR to prevent black backgrounds in Word.
...
// â WRONG: new Paragraph({ heading: HeadingLevel.HEADING_1, style: "customHeader", children: [new TextRun("Title")] })
// â CORRECT: new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun("Title")] })
...
// â ï¸ CRITICAL: NEVER use PageBreak standalone - it will create invalid XML that Word cannot open
// â WRONG: new PageBreak()
// â CORRECT: new Paragraph({ children: [new PageBreak()] })
```
...
- **Symbols:** `"2022"` (â¢), `"00A9"` (Â©), `"00AE"` (Â®), `"2122"` (â¢), `"00B0"` (Â°), `"F070"` (â), `"F0FC"` (â)
