...
function validateDimensions(bodyDimensions, pres) {
  ...
  if (pres.presLayout) {
    ...
    if (
      ...
      errors.push(
        `HTML dimensions (${widthInches.toFixed(1)}" Ã ${heightInches.toFixed(1)}") ` +
          `don't match presentation layout (${layoutWidth.toFixed(1)}" Ã ${layoutHeight.toFixed(1)}")`
      );
    }
  }
  return errors;
...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    const getRotation = (transform, writingMode) => {
      ...
      // PowerPoint: 90Â° = text rotated 90Â° clockwise (reads top to bottom, letters upright)
      // PowerPoint: 270Â° = text rotated 270Â° clockwise (reads bottom to top, letters upright)
      if (writingMode === 'vertical-rl') {
        // vertical-rl alone = text reads top to bottom = 90Â° in PowerPoint
        angle = 90;
      } else if (writingMode === 'vertical-lr') {
        // vertical-lr alone = text reads bottom to top = 270Â° in PowerPoint
        angle = 270;
      }
    ...
    const getPositionAndSize = (el, rect, rotation) => {
      ...
      // For 90Â° or 270Â° rotations, swap width and height
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (el.tagName === 'UL' || el.tagName === 'OL') {
        ...
        liElements.forEach((li, idx) => {
          ...
          if (runs.length > 0) {
            runs[0].text = runs[0].text.replace(/^[â¢\-\*âªâ¸]\s*/, '');
      ...
      if (
        el.tagName !== 'LI' &&
        /^[â¢\-\*âªâ¸âââââ â¡]\s/.test(text.trimStart())
      ) {
        errors.push(
