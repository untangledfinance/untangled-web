...
â   â   âââ test-all.md
â   â   âââ deploy.md
â   âââ skills/            # Project-specific skills
â   â   âââ api-testing/
â   âââ hooks.json         # Hooks configuration
â   âââ mcp.json           # MCP servers (no secrets!)
â   âââ .env.example       # Environment template
âââ .gitignore             # Ignore .claude/.env
âââ README.md
```
...
if (( $(echo "$CURRENT_SPEND > $MONTHLY_BUDGET" | bc -l) )); then
  echo "â ï¸  Monthly budget exceeded: \$$CURRENT_SPEND / \$$MONTHLY_BUDGET"
  exit 1
fi
```
