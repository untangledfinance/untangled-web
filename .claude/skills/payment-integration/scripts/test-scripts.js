...
runner.test('should reject invalid SePay transfer type', () => {
  ...
  runner.assert(
    result.error.includes('Invalid transferType'),
    'Should report invalid transfer type'
  );
});
...
runner.test('should categorize Polar event types', () => {
  runner.assertEqual(
    PolarWebhookVerifier.getEventCategory('order.paid'),
    'order'
  );
  runner.assertEqual(
    PolarWebhookVerifier.getEventCategory('subscription.active'),
    'subscription'
  );
  runner.assertEqual(
    PolarWebhookVerifier.getEventCategory('customer.created'),
    'customer'
  );
  runner.assert(PolarWebhookVerifier.isPaymentEvent('order.paid') === true);
  runner.assert(
    PolarWebhookVerifier.isSubscriptionEvent('subscription.active') === true
  );
});
...
runner.test('should generate SePay checkout fields', () => {
  ...
  runner.assert(
    result.fields.signature !== undefined,
    'Should generate signature'
  );
  runner.assertEqual(
    result.fields.merchant_id,
    'SP-TEST-123',
    'Should include merchant ID'
  );
...
runner.test('should generate Polar checkout config', () => {
  ...
  runner.assertEqual(
    result.config.product_price_id,
    'price_123',
    'Should include price ID'
  );
  runner.assertEqual(
    result.config.external_customer_id,
    'user_123',
    'Should include customer ID'
  );
  runner.assert(
    result.apiEndpoint.includes('sandbox'),
    'Should use sandbox endpoint'
  );
});
...
runner.test('should reject Polar config with relative URL', () => {
  try {
    ...
    runner.assert(
      error.message.includes('absolute URL'),
      'Should require absolute URL'
    );
  }
});
