...
if (require.main === module) {
  ...
  if (args.length < 1) {
    console.log(
      'Usage: node polar-webhook-verify.js <webhook-payload-json> [webhook-secret]'
    );
    console.log(
  ...
  try {
    ...
    if (!secret) {
      console.error('â Error: Webhook secret is required');
      console.error(
    ...
    if (result.success) {
      console.log('â Webhook verified successfully\n');
      console.log('Event Details:');
      console.log(`  Type: ${result.event.type}`);
      console.log(
        `  Category: ${PolarWebhookVerifier.getEventCategory(result.event.type)}`
      );
      console.log(
      ...
      console.error('â Verification failed:', result.error);
      process.exit(1);
    }
  } catch (error) {
