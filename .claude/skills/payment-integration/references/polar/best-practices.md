...
```typescript
// â Good: Set external_id consistently
...
// â Bad: Storing Polar customer IDs
...
// â Bad: PII, passwords, payment details
```
...
```typescript
// â Bad: Trusting webhook data without verification
app.post('/webhook/polar', async (req, res) => {
  await handleEvent(req.body); // Vulnerable!
});

// â Good: Always verify
app.post('/webhook/polar', async (req, res) => {
  const event = validateEvent(req.body, req.headers, secret);
...
```typescript
// â Bad: Fulfilling based on redirect
app.get('/success', async (req, res) => {
  await fulfillOrder(req.query.checkout_id); // Insecure!
});

// â Good: Verify via API or wait for webhook
app.get('/success', async (req, res) => {
...
```typescript
// â Good: Idempotent webhook handling
...
```typescript
// â Bad: Using Polar customer IDs
const customerId = 'cust_abc123';

// â Good: Using your user IDs
