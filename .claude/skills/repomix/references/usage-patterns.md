...
**Workflow:** Package library â enable security checks â review vulnerabilities â check suspicious patterns â AI analysis
...
**Workflow:** Package â AI â OpenAPI/Swagger â endpoint docs â examples
...
**Actions:** Review files â add to `.repomixignore` â remove sensitive data â use env vars
...
**Planning:** Define scope â identify files â check token limits â consider security

**Execution:** Start broad, refine narrow â use appropriate format â enable security checks â monitor tokens

**Review:** Verify no sensitive data â check completeness â validate format â test with LLM

**Iteration:** Refine patterns â adjust format â optimize tokens â document patterns
