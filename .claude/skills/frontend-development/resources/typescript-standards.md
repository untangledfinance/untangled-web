...
// â Or use unknown for truly unknown data
...
```typescript
// â CORRECT - Explicit return type
...
// â AVOID - Implicit return type (less clear)
...
```typescript
// â CORRECT - Explicitly mark as type import
import type { User } from '~types/user';
import type { Post } from '~types/post';
import type { SxProps, Theme } from '@mui/material';

// â AVOID - Mixed value and type imports
...
```typescript
// â OK - When you know more than TypeScript
const element = document.getElementById('my-element') as HTMLInputElement;
const value = element.value;

// â OK - API response that you've validated
...
```typescript
// â AVOID - Circumventing type safety
const data = getData() as any; // WRONG - defeats TypeScript

// â AVOID - Unsafe assertion
...
```typescript
// â OK - When you're certain value exists
const data = queryClient.getQueryData<Data>(['data'])!;

// â ï¸ CAREFUL - Only use when you KNOW it's not null
...
- â No `any` type (use `unknown` if needed)
- â Explicit return types on functions
- â Use `import type` for type imports
- â JSDoc comments on prop interfaces
- â Utility types (Partial, Pick, Omit, Required, Record)
- â Type guards for narrowing
- â Optional chaining and nullish coalescing
- â Avoid type assertions unless necessary
