...
- â Folder-based: `routes/my-route/index.tsx`
- â Lazy load components: `React.lazy(() => import())`
- â Use `createFileRoute` with route path
- â Add breadcrumb in `loader` function
- â Wrap in `SuspenseLoader` for loading states
- â Use `Route.useParams()` for dynamic params
- â Use `useNavigate()` for programmatic navigation
