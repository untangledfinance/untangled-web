...
export const DataDisplay: React.FC<{ items: Item[], searchTerm: string }> = ({
    ...
    // â AVOID - Runs on every render
    const filteredItems = items
        .filter(item => item.name.includes(searchTerm))
        .sort((a, b) => a.name.localeCompare(b.name));

    // â CORRECT - Memoized, only recalculates when dependencies change
...
```typescript
// â AVOID - Creates new function on every render
...
export const Parent: React.FC = () => {
    // â CORRECT - Stable function reference
...
// â Wrap expensive components in React.memo
...
export const MyForm: React.FC = () => {
    ...
    // â AVOID - Watches all fields, re-renders on any change
    const formValues = watch();

    // â CORRECT - Watch only what you need
...
// â AVOID - Index as key (unstable if list changes)
...
```typescript
// â AVOID - Component recreated on every render
export const Parent: React.FC = () => {
...
```typescript
// â CORRECT - Define outside or use useMemo
...
```typescript
// â AVOID - Import heavy libraries at top level
import jsPDF from 'jspdf';
// Large library loaded immediately
import * as XLSX from 'xlsx';

// Large library loaded immediately

// â CORRECT - Dynamic import when needed
...
- â `useMemo` for expensive computations (filter, sort, map)
- â `useCallback` for functions passed to children
- â `React.memo` for expensive components
- â Debounce search/filter (300-500ms)
- â Cleanup timeouts/intervals in useEffect
- â Watch specific form fields (not all)
- â Stable keys in lists
- â Lazy load heavy libraries
