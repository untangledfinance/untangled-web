...
```typescript
// â CORRECT - TanStack Query for server data
...
```typescript
// â CORRECT - useState for UI state
...
- â useAuth hook for current user (id, email, roles, username)
- â React Hook Form + Zod for forms
- â Dialog with icon + close button
- â DataGrid wrapper contracts
- â Mutations with cache invalidation
- â TanStack Query for server state
- â useState for UI state
- â Zustand for global client state (minimal)
