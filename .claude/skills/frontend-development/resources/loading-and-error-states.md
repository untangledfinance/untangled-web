...
## â ï¸ CRITICAL RULE: Never Use Early Returns

### The Problem

```typescript
// â NEVER DO THIS - Early return with loading spinner
const Component = () => {
...
```typescript
// â NEVER - Early return
if (isLoading) {
    return <CircularProgress />;
}

// â NEVER - Conditional rendering
{isLoading ? <Spinner /> : <Content />}

// â NEVER - Layout changes
if (isLoading) {
    return (
        <Box sx={{ height: 100 }}>
            <Spinner />
...
```typescript
// â BEST - useSuspenseQuery + SuspenseLoader
<SuspenseLoader>
    <ComponentWithSuspenseQuery />
</SuspenseLoader>

// â ACCEPTABLE - LoadingOverlay
<LoadingOverlay loading={isLoading}>
    <Content />
</LoadingOverlay>

// â OK - Inline skeleton with same layout
<Box sx={{ height: 500 }}>
...
- â **PREFERRED**: SuspenseLoader + useSuspenseQuery (modern pattern)
- â **ACCEPTABLE**: LoadingOverlay (legacy pattern)
- â **OK**: Skeleton with same layout
- â **NEVER**: Early returns or conditional layout

**Error Handling:**

- â **ALWAYS**: useMuiSnackbar for user feedback
- â **NEVER**: react-toastify
- â Use onError callbacks in queries/mutations
- â Error boundaries for component-level errors
