...
echo "â commit: $HASH $(git log -1 --pretty=%s)" && \
if git push 2>&1; then echo "â pushed: yes"; else echo "â pushed: no (run 'git push' manually)"; fi
```
...
- â "ð¤ Generated with [Claude Code]"
- â "Co-Authored-By: Claude <noreply@anthropic.com>"
- â "AI-assisted commit"
- â Any AI tool attribution, signature, or reference
...
- â `Updated some files` (not descriptive)
- â `feat(auth): added user login validation using bcrypt library with salt rounds` (too long, describes HOW)
- â `Fix bug` (not specific enough)
...
```
â staged: 3 files (+45/-12 lines)
â security: passed
â commit: a3f8d92 feat(auth): add token refresh
â pushed: yes
```
...
| ------------------ | --------------------------------------------- | ---------------------------------------- |
| Secrets detected   | "â Secrets found in: [files]" + matched lines | Block commit, suggest .gitignore         |
| No changes staged  | "â No changes to commit"                      | Exit cleanly                             |
| Nothing to add     | "â No files modified"                         | Exit cleanly                             |
| Merge conflicts    | "â Conflicts in: [files]"                     | Suggest `git status` â manual resolution |
| Push rejected      | "â  Push rejected (out of sync)"              | Suggest `git pull --rebase`              |
