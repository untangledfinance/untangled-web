...
```
Loading .env file...
â Environment loaded, DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/12...
...
â Added user authentication
â Created login/signup forms
â Integrated JWT tokens
â All tests passing'
```
...
if npm run build && npm run test; then
    ./.claude/hooks/send-discord.sh 'â Build and tests passed - ready to deploy'
else
    ./.claude/hooks/send-discord.sh 'â Build or tests failed - deployment blocked'
fi
```
...
{
  "scripts": {
    "deploy": "npm run build && npm run test && ./.claude/hooks/send-discord.sh 'â Deployment successful'",
...
- **Title:** ð¤ Claude Code Session Complete
...
- ð **Project:** Current project directory name

**Example Discord Message:**

```
âââââââââââââââââââââââââââââââââ
â ð¤ Claude Code Session Complete
â ââââââââââââââââââââââââââââââââ£
â Implementation Complete
â
â â Added user authentication
â â Created login/signup forms
â â Integrated JWT tokens
â â All tests passing
â ââââââââââââââââââââââââââââââââ£
â â° Session Time: 14:30:45
â ð Project: untangled-web
â ââââââââââââââââââââââââââââââââ£
â DevPocket API â¢ untangled-web
â Today at 14:30
âââââââââââââââââââââââââââââââââ
```
...
if npm run test; then
    if [[ "$ENVIRONMENT" == "production" ]]; then
        ./.claude/hooks/send-discord.sh 'â Production deployment successful'
    fi
else
    # Always notify on failures
    ./.claude/hooks/send-discord.sh 'â Tests failed - deployment aborted'
fi
```
