...
case "$HOOK_TYPE" in
    "Stop")
        ...
        MESSAGE="ð *DevPocket Task Completed*
        
ð *Time:* ${TIMESTAMP}
ð *Project:* ${PROJECT_NAME}
ð§ *Total Operations:* ${TOTAL_TOOLS}
ð *Session:* ${SESSION_ID:0:8}...
...
ð *Location:* \`${PROJECT_DIR}\`"
        ;;
        
    "SubagentStop")
        SUBAGENT_TYPE=$(echo "$INPUT" | jq -r '.subagentType // "unknown"')
        MESSAGE="ð¤ *DevPocket Subagent Completed*

ð *Time:* ${TIMESTAMP}
ð *Project:* ${PROJECT_NAME}
ð§ *Agent Type:* ${SUBAGENT_TYPE}
ð *Session:* ${SESSION_ID:0:8}...
...
ð *Location:* \`${PROJECT_DIR}\`"
    ...
    *)
        MESSAGE="ð *DevPocket Code Event*
