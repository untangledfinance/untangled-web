...
if [[ -z "$DISCORD_WEBHOOK_URL" ]]; then
    echo "â ï¸  Discord notification skipped: DISCORD_WEBHOOK_URL not set"
    exit 1
fi
...
"embeds": [{
    ...
    "footer": {
        "text": "DevPocket API â¢ $(basename "$(pwd)")"
    },
    "fields": [
        {
            "name": "â° Session Time",
...
if [[ $? -eq 0 ]]; then
    echo "â Discord notification sent"
else
    echo "â Failed to send Discord notification"
    exit 1
fi