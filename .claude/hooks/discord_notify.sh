...
if [[ -z "$DISCORD_WEBHOOK_URL" ]]; then
    echo "â ï¸  Discord notification skipped: DISCORD_WEBHOOK_URL not set" >&2
    exit 0
fi
    ...
    local payload=$(cat <<EOF
{
    "embeds": [{
        ...
        "footer": {
            "text": "DevPocket API â¢ ${PROJECT_NAME}"
        },
        "fields": $fields
    }]
}
...
case "$HOOK_TYPE" in
    "Stop")
        ...
        # Build description
        DESCRIPTION="â Claude Code session completed successfully"
...
EOF
        ...
        send_discord_embed "ð¤ Claude Code Session Complete" "$DESCRIPTION" 5763719 "$FIELDS"
        ;;
...
EOF
        ...
        send_discord_embed "ð¯ Claude Code Subagent Complete" "$DESCRIPTION" 3447003 "$FIELDS"
        ;;
...
EOF
        ...
        send_discord_embed "ð Claude Code Event" "$DESCRIPTION" 10070709 "$FIELDS"
        ;;
esac

# Log the notification (optional)
echo "â Discord notification sent for $HOOK_TYPE event in project $PROJECT_NAME" >&2
